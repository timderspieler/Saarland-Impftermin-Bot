...
public class Impfstellen extends JFrame {
	...
	public Impfstellen() {
		...
		for (City all : City.values()) {
			model.addElement(all.getName() + ": (" + all.getVaccinationType() + ")");
		}
		
		list.setToolTipText("Wählen Sie eine Impfstelle");
		...
		button_2_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String msg = "Sie müssen zuerst eine Impfstelle auswählen!";
				
				if (select != null) {
					
					String selected_city = select.split(":")[0];
					msg = "Impfstelle konnte nicht gespeichert werden!";
					
					City c = City.getByName(selected_city);
					
					if (c != null) {
						Main.c = c;
						msg = "Impfstelle wurde gespeichert!\n(" + c.getName() +", " + c.getVaccinationType() + ")";
					}
